...
void setup() {
  ...
  BLEDevice::init("M5StickC_Server");
...
void loop() {
  ...
  if (M5.BtnA.wasPressed()) {
    if (!displayOn) {
      M5.Axp.SetLDO2(true);  // Riaccendi lo schermo
      displayOn = true;
  ...
  // Spegni lo schermo dopo 20 secondi di inattività
  if (displayOn && millis() - lastActivityTime > 20000) {
    M5.Axp.SetLDO2(false);  // Spegni lo schermo
    displayOn = false;
  }

  // Aggiorna la visualizzazione solo se lo stato della connessione cambia
...
void updateDisplay() {
    ...
    M5.Lcd.fillRect(0, 0, M5.Lcd.width(), 10, deviceConnected ? TFT_GREEN : TFT_RED);

    // Disegna una banda colorata in basso
    M5.Lcd.fillRect(0, M5.Lcd.height() - 10, M5.Lcd.width(), 10, deviceConnected ? TFT_GREEN : TFT_RED);
